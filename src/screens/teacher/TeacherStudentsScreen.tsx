...
export function TeacherStudentsScreen() {
  ...
  const filtered = (students ?? []).filter((st) => {
    ...
    if (search.trim()) {
      ...
      return st.name.toLowerCase().includes(term) || (st.rollNumber ?? "").toLowerCase().includes(term);
    }
    return true;
  });
    ...
    <View style={{ flex: 1, backgroundColor: D.bg }}>
      <ScrollView contentContainerStyle={{ paddingBottom: 140 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.contentArea}>
          {/* Summary card */}
          <View style={[s.featureCard, { marginBottom: 24 }]}>
            ...
            <Text style={s.fcValue}>{loading ? "—" : String(students?.length ?? "—")}</Text>
            <Text style={s.fcSub}>{classNames.length} batch{classNames.length !== 1 ? "es" : ""}</Text>
          </View>

          <View style={s.sectionHeader}>
            <Text style={s.sectionTitle}>
              {selectedBatch === "All" ? "All students" : selectedBatch}
            </Text>
            {!loading && <Text style={s.sectionCount}>{filtered.length}</Text>}
          </View>
...
const s = StyleSheet.create({
  ...
  featureCard: { backgroundColor: "#fff", borderRadius: 12, padding: 14, borderWidth: 1, borderColor: D.outlineVariant, shadowColor: D.primary, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.025, shadowRadius: 4, elevation: 1 },
  ...
  sectionCount: { fontSize: 11, fontWeight: "600", fontFamily: D.fontSemiBold, color: D.outline },
