        ...
        <View style={s.contentArea}>
          <View style={s.grid2}>
            <View style={s.featureCard}>
              ...
              <Text style={s.fcLabel}>ALL STUDENTS</Text>
              <Text style={s.fcValue}>{loading ? "—" : String(students?.length ?? 0)}</Text>
              <Text style={s.fcSub}>{classNames.length} batch{classNames.length !== 1 ? "es" : ""}</Text>
            </View>
            <View style={s.featureCard}>
              <View style={s.fcTopRow}>
                <View style={[s.fcIcon, { backgroundColor: "#F0FDF4" }]}>
                  <Ionicons name="school-outline" size={14} color="#15803D" />
                </View>
              </View>
              <Text style={s.fcLabel}>SHOWING</Text>
              <Text style={s.fcValue}>{loading ? "—" : String(filtered.length)}</Text>
              <Text style={s.fcSub}>{selectedBatch === "All" ? "All batches" : selectedBatch}</Text>
            </View>
          </View>
            ...
            <View style={s.card}>
              {filtered.map((st, i) => (
                <AnimatedPressable
                  ...
                  onPress={() =>
                    router.push({
                      pathname: "/(employee)/student-detail",
