...
const menuItems: { label: string; icon: IoniconsName; route?: string; destructive?: boolean }[] = [
  { label: "Notifications", icon: "notifications-outline", route: "/(employee)/notifications" },
...
export function EmployeeAccountScreen() {
  ...
  const initials = profile?.name?.split(" ").slice(0, 2).map((w: string) => w[0]).join("") || "EO";
  const employeeType = (profile as any)?.employeeType || "Office Staff";
  const employeeId = (profile as any)?.employeeId || (profile as any)?.employee_id || "";
  const centre = (profile as any)?.centreName || "";
      ...
      >
        ...
        <View style={s.profileBlock}>
          ...
          <View style={s.profileInfo}>
            <Text style={s.profileName}>{profile?.name || "Employee"}</Text>
            <View style={s.profileRoleRow}>
              <View style={s.roleChip}>
                <Text style={s.roleChipText}>Office Staff</Text>
              </View>
            </View>
            {!!employeeType && <Text style={s.profileSubjects}>{employeeType}</Text>}
            {!!centre && <Text style={s.profileClasses}>{centre}</Text>}
          </View>
        </View>
        ...
        <View style={s.grid2}>
          {[
            ...
            { label: "Centre", value: centre ? centre.split(" ").slice(-1)[0] || "—" : "—" },
          ].map((stat) => (
...
const s = StyleSheet.create({
  ...
  statValue: { fontSize: 14, fontWeight: "800", color: D.onSurface, letterSpacing: -0.3, fontFamily: D.fontExtraBold },
