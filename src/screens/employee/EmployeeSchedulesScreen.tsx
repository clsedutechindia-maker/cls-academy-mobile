...
export function EmployeeSchedulesScreen() {
  ...
  const { data, loading, error, reload } = useResource(
    async () => {
      ...
      return listEmployeeTimetable(profile);
    },
    [profile?.userId],
  );
  ...
  function openCreate() {
    ...
    if (activeTab === 0) {
      router.push({ pathname: "/(employee)/timetable-editor", params: { classId: selectedClassId, mode: "create", day: selectedDay } });
    } else {
      router.push({ pathname: "/(employee)/exam-editor", params: { classId: selectedClassId, mode: "create" } });
    }
  }

  function openEditSlot(entry: ClassTimetableRecord) {
    router.push({ pathname: "/(employee)/timetable-editor", params: { classId: entry.classId, entryId: entry.id, mode: "edit", day: entry.dayKey } });
  }

  function openEditExam(entry: TestScheduleRecord) {
    router.push({ pathname: "/(employee)/exam-editor", params: { classId: entry.classId, entryId: entry.id, mode: "edit" } });
  }
          ...
          <>
            ...
            {todaySlots.length === 0 ? (
              <View style={[s.card, { padding: 24, alignItems: "center" }]}>
                ...
                <Text style={{ marginTop: 10, fontFamily: D.font, color: D.outline, fontSize: 13 }}>No classes scheduled. Tap + to add.</Text>
              </View>
            ) : (
              <View style={s.card}>
                {todaySlots.map((sl, i) => {
          ...
          <>
            ...
            {upcomingTests.length === 0 ? (
              <View style={[s.card, { padding: 24, alignItems: "center" }]}>
                ...
                <Text style={{ marginTop: 10, fontFamily: D.font, color: D.outline, fontSize: 13 }}>No upcoming exams. Tap + to schedule.</Text>
              </View>
            ) : (
              <View style={s.card}>
