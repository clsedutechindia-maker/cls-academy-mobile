...
import { listEmployeeResults } from "../../lib/erp";
...
const subjectColor: Record<string, { text: string; bg: string }> = {
  ...
  Math: { text: "#F59E0B", bg: "#FFFBEB" },
};

function getSubjectColor(name: string) {
  for (const key of Object.keys(subjectColor)) {
    if (name.includes(key)) return subjectColor[key]!;
  }
...
export function EmployeeResultsScreen() {
  ...
  const filteredAll = selectedBatch === "All batches" ? (results ?? []) : (results ?? []).filter((r) => r.className === selectedBatch);
  const avgPct = filteredAll.length > 0
    ? Math.round(filteredAll.reduce((acc, r) => acc + (r.maxScore > 0 ? (r.score / r.maxScore) * 100 : 0), 0) / filteredAll.length)
    : 0;
    ...
    <View style={{ flex: 1, backgroundColor: D.bg }}>
      <ScrollView contentContainerStyle={{ paddingBottom: 160 }} showsVerticalScrollIndicator={false}>
        <View style={[s.headerSection, { paddingTop: insets.top + 20 }]}>
          ...
          {searchVisible && (
            <View style={s.searchBar}>
              ...
              <TextInput style={s.searchInput} placeholder="Search results…" placeholderTextColor={D.outline} value={search} onChangeText={setSearch} autoFocus />
              {search.length > 0 && <Pressable onPress={() => setSearch("")}><Ionicons name="close-circle" size={17} color={D.outline} /></Pressable>}
            </View>
          )}
        </View>
        ...
        <View style={s.contentArea}>
          <View style={s.grid2}>
            {[
              { label: "Tests Entered", value: loading ? "—" : String(uniqueTests.length), sub: "Unique assessments", icon: "document-text-outline" as const, tone: { bg: D.surfaceLow, fg: D.primary } },
          ...
          <View style={s.sectionHeader}>
            <Text style={s.sectionTitle}>Uploaded results</Text>
          </View>

          {loading && <View style={[s.card, { padding: 20, alignItems: "center" }]}><Text style={{ fontSize: 13, fontFamily: D.font, color: D.outline }}>Loading…</Text></View>}
              ...
              {filtered.map((r, i) => {
                ...
                return (
                  <AnimatedPressable
                    ...
                    onPress={() => router.push({
                      pathname: "/(employee)/result-detail",
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={s.resultMeta}>{r.className}{r.assessmentDate ? ` · ${r.assessmentDate}` : ""}</Text>
                    </View>
      ...
      <Modal visible={pickerOpen} transparent animationType="fade" onRequestClose={() => setPickerOpen(false)}>
        <Pressable style={s.modalOverlay} onPress={() => setPickerOpen(false)}>
          <View style={s.modalSheet}>
            ...
            {batches.map((b) => (
              <Pressable key={b} style={[s.modalOption, b === selectedBatch && s.modalOptionActive]} onPress={() => { setSelectedBatch(b); setPickerOpen(false); }}>
