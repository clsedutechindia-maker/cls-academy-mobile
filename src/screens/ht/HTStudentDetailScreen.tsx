...
function ResultsTab({ userId }: { userId: string }) {
  ...
  return (
    <>
      <View style={[s.rankCard, { marginTop: 18 }]}>
        <View style={{ flex: 1 }}>
      ...
      <Text style={[s.sectionLabel, { marginTop: 28 }]}>SUBJECT PERFORMANCE</Text>
      <View style={{ gap: 10 }}>
        {Object.entries(bySubject).map(([subjName, subjResults]) => {
          ...
          return (
            <View key={subjName} style={[s.card, { padding: 16 }]}>
              <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                  <View style={{ width: 8, height: 8, borderRadius: 2, backgroundColor: sc.color }} />
                  <Text style={{ fontSize: 12.5, fontWeight: "700", fontFamily: D.fontBold, color: D.onSurface }}>{subjName}</Text>
                  <View style={[s.badge, { backgroundColor: sc.bg }]}>
                    <Text style={[s.badgeText, { color: sc.color }]}>{subjResults.length} tests</Text>
                  </View>
                </View>
                <Text style={{ fontSize: 14, fontWeight: "800", fontFamily: D.fontExtraBold, color: sc.color, letterSpacing: -0.4 }}>{avg}%</Text>
              </View>
              <View style={{ height: 6, borderRadius: 999, backgroundColor: sc.bg, overflow: "hidden" }}>
      ...
      <Text style={[s.sectionLabel, { marginTop: 28 }]}>RECENT TESTS</Text>
      <View style={s.card}>
          ...
          return (
            <View key={t.id} style={[s.testResultRow, i < Math.min(results.length, 10) - 1 && s.divider]}>
              <View style={[s.subjectIcon, { backgroundColor: sc.bg }]}>
                <Text style={{ fontSize: 9.5, fontWeight: "800", fontFamily: D.fontExtraBold, color: sc.color }}>
              ...
              <View style={{ alignItems: "flex-end" }}>
                <Text style={s.scoreText}>
                  {t.score}
                  <Text style={{ fontSize: 10.5, color: D.outline, fontWeight: "500", fontFamily: D.fontMedium }}>/{t.maxScore}</Text>
                </Text>
                <Text style={{ fontSize: 10.5, fontWeight: "700", fontFamily: D.fontBold, color: pct >= 80 ? "#15803D" : pct >= 60 ? "#B45309" : "#B91C1C", marginTop: 2 }}>{pct}%</Text>
              </View>
            </View>
          );
        })}
...
const s = StyleSheet.create({
  ...
  badgeText: { fontSize: 10, fontWeight: "700", fontFamily: D.fontBold, letterSpacing: 0.3 },
  ...
  infoLabel: { width: 120, fontSize: 12, fontWeight: "600", fontFamily: D.fontSemiBold, color: D.outline, letterSpacing: 0.1, flexShrink: 0 },
  infoValue: { flex: 1, minWidth: 0, flexShrink: 1, fontSize: 13, fontWeight: "500", fontFamily: D.fontMedium, color: D.onSurface, letterSpacing: -0.1, textAlign: "right" },
  ...
  rankSub: { fontSize: 11, fontFamily: D.font, color: "rgba(255,255,255,0.8)", marginTop: 4 },
  sectionLabel: { fontSize: 10, fontWeight: "700", fontFamily: D.fontBold, color: D.outline, letterSpacing: 0.5, marginBottom: 14 },
  subjectIcon: { width: 36, height: 36, borderRadius: 11, alignItems: "center", justifyContent: "center", flexShrink: 0 },
  testResultRow: { flexDirection: "row", alignItems: "center", gap: 14, padding: 16 },
  testName: { fontSize: 12.5, fontWeight: "700", fontFamily: D.fontBold, color: D.onSurface, letterSpacing: -0.1 },
  testMeta: { fontSize: 11, fontFamily: D.font, color: D.outline, marginTop: 2 },
  scoreText: { fontSize: 12.5, fontWeight: "800", fontFamily: D.fontExtraBold, color: D.onSurface, letterSpacing: -0.3 },
