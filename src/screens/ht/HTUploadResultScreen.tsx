...
export function HTUploadResultScreen() {
  ...
  // Unique class names from mappings
  const batchNames = Array.from(new Set((mappings ?? []).map((m) => m.className).filter(Boolean)));

  // Subjects for selected batch
  const subjectsForBatch: ClassSubjectRecord[] = (mappings ?? []).filter(
    (m) => m.className === selectedBatch,
  );

  // Students for selected batch
  const studentsForBatch: UserProfileRecord[] = (allStudents ?? []).filter(
    (s) => s.className === selectedBatch,
  );
    ...
    return (
      <View style={{ flex: 1, backgroundColor: D.bg, alignItems: "center", justifyContent: "center", padding: 32 }}>
        ...
        <Text style={{ fontSize: 13, fontFamily: D.font, color: D.onSurfaceVariant, textAlign: "center", marginBottom: 24 }}>
          Results for {testName} have been uploaded successfully.
        </Text>
          ...
          <>
            ...
            <View style={{ marginBottom: 14 }}>
              ...
              <TextInput
                ...
                placeholder="e.g. Cell Division Unit Test"
            ...
            {selectedBatch !== "" && studentsForBatch.length > 0 && (
              <>
                <Text style={s.sectionLabel}>
                  STUDENT MARKS · {studentsForBatch.length} STUDENTS
                </Text>
                <View style={s.card}>
                  <View style={[s.tableHeader, { backgroundColor: D.bg }]}>
                    <Text style={s.colLabel}>STUDENT</Text>
                    <Text style={[s.colLabel, { width: 80, textAlign: "center" }]}>
                ...
                {missingStudents.length > 0 && (
                  <View style={s.warnBanner}>
                    ...
                    <Text style={s.warnText}>
                      {missingStudents.length} student{missingStudents.length > 1 ? "s" : ""} missing marks
                    </Text>
                  </View>
                )}
              </>
          ...
          <View style={s.modalSheet}>
            ...
            {batchNames.map((b) => (
              <Pressable
                ...
                onPress={() => { setSelectedBatch(b); setSelectedSubjectId(""); setMarks({}); setBatchPickerOpen(false); }}
              >
        ...
        <Pressable style={s.modalOverlay} onPress={() => setSubjectPickerOpen(false)}>
          <View style={s.modalSheet}>
            ...
            {subjectsForBatch.map((m) => (
              <Pressable
                key={m.id}
                style={[s.modalOption, m.id === selectedSubjectId && s.modalOptionActive]}
                onPress={() => { setSelectedSubjectId(m.id); setSubjectPickerOpen(false); }}
              >
